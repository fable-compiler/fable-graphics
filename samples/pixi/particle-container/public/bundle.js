...
for(var symbols = $keys(wks.store), i$1 = 0; symbols.length > i$1; )wksDefine(symbols[i$1++]);
...
var patternInput_21 = function () {
  var matchValue = PIXI.autoDetectRenderer(800, 600);

  if (matchValue instanceof PIXI.CanvasRenderer) {
...
var renderer = patternInput_21[1];
var isWebGL = patternInput_21[0];
...
var sprites = new PIXI.ParticleContainer(10000, options);
stage.addChild(sprites);
var maggots = [];
var totalSprites = isWebGL ? 10000 : 100;

for (var i = 0; i <= totalSprites - 1; i++) {
  var dude = PIXI.Sprite.fromImage("./public/assets/tinyMaggot.png");
  dude.anchor.set(0.5);
...
var animate = function () {
  ...
  var animate_1 = function animate_1(dt) {
    ...
    for (var _i = 0; _i <= dc; _i++) {
      var _dude = maggots[_i];
      _dude.scale.y = 0.95 + Math.sin(tick + _dude.offset) * 0.05;
      _dude.direction = _dude.direction + _dude.turningSpeed * 0.01;
      _dude.position.x = _dude.position.x + Math.sin(_dude.direction) * _dude.speed * _dude.scale.y;
      _dude.position.y = _dude.position.y + Math.cos(_dude.direction) * _dude.speed * _dude.scale.y;
      _dude.rotation = -_dude.direction - Math.PI;

      if (_dude.position.x < dudeBounds.x) {
        _dude.position.x = _dude.position.x + dudeBounds.width;
      } else {
        if (_dude.position.x > dudeBounds.x + dudeBounds.width) {
          _dude.position.x = _dude.position.x - dudeBounds.width;
        }
      }

      if (_dude.position.y < dudeBounds.y) {
        _dude.position.y = _dude.position.y + dudeBounds.height;
      } else {
        if (_dude.position.y > dudeBounds.y + dudeBounds.height) {
          _dude.position.y = _dude.position.y - dudeBounds.height;
        }
      }
    }
...
exports.maggots = maggots;
exports.totalSprites = totalSprites;
exports.dudeBoundsPadding = dudeBoundsPadding;
exports.dudeBounds = dudeBounds;
exports.animate = animate;

}((this.particle_container = this.particle_container || {}),PIXI));
